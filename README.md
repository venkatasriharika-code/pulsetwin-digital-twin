...
The repository includes `vercel.json`, `.vercelignore`, and `api/index.py`. Keep the Vercel Root Directory set to the repository root: the React/Vite frontend builds from `frontend`, while `/api/index.py` exposes the FastAPI application and the root `requirements.txt` points Vercel to the backend dependencies. Set `VITE_API_BASE_URL` only when the API is deployed separately; with the root full-stack deployment, the frontend can use the same Vercel origin for API routes. The local development default remains `http://localhost:8000`.
