...
The repository includes `vercel.json` and `.vercelignore` for deploying the React/Vite frontend from the `frontend` directory without Vercel detecting the Python backend as the frontend build. Import this repository into Vercel with the default project settings; Vercel will run the frontend build and serve the SPA routes correctly. Set `VITE_API_BASE_URL` in the Vercel project environment variables to the deployed FastAPI backend URL so the live simulation, alerts, recommendations, persistence, and model APIs work in production. The local development default remains `http://localhost:8000`.
